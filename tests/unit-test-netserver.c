...
extern int get_ipstring(char *iface, char *ipstr, int strlen);
int main(int argc, char*argv[])
{
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "tcp") == 0) {
            ...
            printf("Usage:\n  %s [tcp|tcppi|rtu] [lan-if] [ip-addr] - Modbus server for unit testing\n\n", argv[0]);
            return -1;
        }
    } else {
        /* By default */
	...
	if (argc>2)
		lanif =  argv[2];
	if (argc==4)
        	get_ipstring(lanif, hostip_str, sizeof(hostip_str));
	else
