...
static int _modbus_tcp_connect(modbus_t *ctx)
{
    ...
    if (rc <= 0) {
        if (ctx->debug) {
            modbus_trace_error("Invalid IP address: %s\n", ctx_tcp->ip);
        }
        close(ctx->s);
        ctx->s = -1;
        return -1;
...
int modbus_tcp_accept(modbus_t *ctx, int *s)
{
    ...
    if (ctx->debug) {
        ...
        if (inet_ntop(AF_INET, &(addr.sin_addr), buf, INET_ADDRSTRLEN) == NULL) {
            modbus_trace_error("Client connection accepted from unparsable IP.\n");
        } else {
            modbus_trace("Client connection accepted from %s.\n", buf);
        }
    }
...
modbus_t *modbus_new_tcp(const char *ip, int port)
{
    ...
    if (sigaction(SIGPIPE, &sa, NULL) < 0) {
        ...
        modbus_trace_error("Could not install SIGPIPE handler.\n");
        return NULL;
    }
#endif
