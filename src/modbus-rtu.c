...
static int _modbus_rtu_receive(modbus_t *ctx, uint8_t *req, int req_max)
{
    int rc;
    modbus_rtu_t *ctx_rtu = ctx->backend_data;

    if (ctx_rtu->confirmation_to_ignore) {
        _modbus_receive_msg(ctx, req, MSG_CONFIRMATION, req_max);
        ...
        rc = _modbus_receive_msg(ctx, req, MSG_INDICATION, req_max);
        if (rc == 0) {
