...
#if HAVE_DECL_TIOCM_RTS
    modbus_serial_t *ctx_serial = ctx->backend_data;
    if (ctx_serial->rts != _MODBUS_BACKEND_TYPE_SERIAL) {
        ssize_t size;
        ...
        _modbus_serial_ioctl_rts(ctx->s, ctx_serial->rts == MODBUS_SERIAL_RTS_UP);
        usleep(_MODBUS_SERIAL_TIME_BETWEEN_RTS_SWITCH);

        size = write(ctx->s, req, req_length);

        usleep(ctx_serial->onebyte_time * req_length + _MODBUS_SERIAL_TIME_BETWEEN_RTS_SWITCH);
        _modbus_serial_ioctl_rts(ctx->s, ctx_serial->rts != MODBUS_SERIAL_RTS_UP);
...
int modbus_serial_set_rts(modbus_t *ctx, int mode)
{
    ...
    if (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_SERIAL) {
        ...
        if (mode == MODBUS_RTU_RTS_NONE || mode == MODBUS_RTU_RTS_UP ||
            ...
            _modbus_serial_ioctl_rts(ctx->s, ctx_serial->rts != MODBUS_RTU_RTS_UP);
...
{
  if(ctx_serial) free(ctx_serial->device);
  free(ctx_serial);
}
