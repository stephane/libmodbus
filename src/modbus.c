                                          ...
                                          msg_type_t msg_type)
{
    ...
    if (msg_type == MSG_INDICATION) {
        ...
        if (function <= _FC_READ_INPUT_REGISTERS ||
            ...
            function == _FC_WRITE_FILE_RECORDS ||
            function == _FC_READ_FILE_RECORDS ||
                              ...
                              uint8_t *rsp, int rsp_length)
{
    ...
    if (rsp_length == rsp_length_computed ||
        ...
        case _FC_WRITE_FILE_RECORDS:
            ...
            break;
        case _FC_READ_FILE_RECORDS:
            /* Read and write file records, first byte tells the lenght. */
        case _FC_REPORT_SLAVE_ID:
...
/* Write set of values from array into modbus file-record 
   In addition to addr there also is a record number. 
   This implementation is the simplistic version with only one file record per request. */
int modbus_write_file_record(modbus_t *ctx, int addr, int sub_addr, int nb, const uint16_t *src)
{
    ...
    if (nb > MODBUS_MAX_WRITE_FILE_RECORDS) {
        if (ctx->debug) {
            fprintf(stderr,
                    "ERROR Trying to write to too many file records (%d > %d)\n",
                    nb, MODBUS_MAX_WRITE_FILE_RECORDS);
        }
        errno = EMBMDATA;
        return -1;
    }
    req_length = ctx->backend->build_request_basis(ctx,
                                                   _FC_WRITE_FILE_RECORDS,
                                                   0, 0, req);
    req_length -= 4;
    
    byte_count = 7 + (nb<<1);
    req[req_length++] = byte_count;         /* request data lenght */
    req[req_length++] = 0x06;               /* "reference type", constant */
    req[req_length++] = addr >> 8;          /* file number */
    req[req_length++] = addr & 0x00ff;
    req[req_length++] = sub_addr >> 8;      /* record number */
    req[req_length++] = sub_addr & 0x00ff;
    req[req_length++] = nb >> 8;            /* number of records */
...
/* Read set of values to array from modbus file-record 
   In addition to addr there also is a record number. 
   This implementation is the simplistic version with only one file record per request. */
int modbus_read_file_record(modbus_t *ctx, int addr, int sub_addr, int nb, uint16_t *dest)
{
    ...
    if (nb > MODBUS_MAX_READ_FILE_RECORDS && nb != 0xffff) {
        if (ctx->debug) {
            fprintf(stderr,
                    "ERROR Trying to read to too many file records (%d > %d)\n",
                    nb, MODBUS_MAX_WRITE_FILE_RECORDS);
        }
        errno = EMBMDATA;
        return -1;
    ...
    if (rc > 0) {
        ...
        rc = (rc >> 1) - 1; /* Bytecount is in words and is prefixed by two bytes. */
        offset = ctx->backend->header_length;
...
void modbus_free(modbus_t *ctx)
{
    ...
    allocators.free(ctx->backend_data);
    allocators.free(ctx);
}
...
/* Set the allocators used by libmodbus. Must be called before  */
void modbus_set_allocators(
  void* (*malloc)(size_t),
                                     ...
                                     int nb_registers, int nb_input_registers)
{
    ...
    mb_mapping = (modbus_mapping_t *)allocators.malloc(sizeof(modbus_mapping_t));
    if (mb_mapping == NULL) {
        return NULL;
    }
    ...
    if (nb_bits == 0) {
        ...
        mb_mapping->tab_bits =
            (uint8_t *) allocators.malloc(nb_bits * sizeof(uint8_t));
        if (mb_mapping->tab_bits == NULL) {
            allocators.free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_input_bits == 0) {
        ...
        mb_mapping->tab_input_bits =
            (uint8_t *) allocators.malloc(nb_input_bits * sizeof(uint8_t));
        if (mb_mapping->tab_input_bits == NULL) {
            allocators.free(mb_mapping->tab_bits);
            allocators.free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_registers == 0) {
        ...
        mb_mapping->tab_registers =
            (uint16_t *) allocators.malloc(nb_registers * sizeof(uint16_t));
        if (mb_mapping->tab_registers == NULL) {
            allocators.free(mb_mapping->tab_input_bits);
            allocators.free(mb_mapping->tab_bits);
            allocators.free(mb_mapping);
            return NULL;
        }
    ...
    if (nb_input_registers == 0) {
        ...
        mb_mapping->tab_input_registers =
            (uint16_t *) allocators.malloc(nb_input_registers * sizeof(uint16_t));
        if (mb_mapping->tab_input_registers == NULL) {
            allocators.free(mb_mapping->tab_registers);
            allocators.free(mb_mapping->tab_input_bits);
            allocators.free(mb_mapping->tab_bits);
            allocators.free(mb_mapping);
            return NULL;
...
void modbus_mapping_free(modbus_mapping_t *mb_mapping)
{
    ...
    allocators.free(mb_mapping->tab_input_registers);
    allocators.free(mb_mapping->tab_registers);
    allocators.free(mb_mapping->tab_input_bits);
    allocators.free(mb_mapping->tab_bits);
    allocators.free(mb_mapping);
